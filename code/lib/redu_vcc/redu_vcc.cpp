...
void redu_vcc::buildCover() {

  // for (redu_vcc &child : chilren) {
    // child.buildCover();
  //
  // }
}
...
void redu_vcc::generateReducedAdjList(std::vector<int> &node_to_int_map, std::vector<NodeID> &int_to_node_map,
                                      std::vector<std::vector<int>> &int_adj_list, unsigned long &edges_count) {

    int int_node = 0;
    for (NodeID v = 0; v < num_nodes; v++) {
      if (!node_status[v]) continue;
      node_to_int_map[v] = int_node;
      int_to_node_map[int_node] = v;
      int_node++;
    }
    ...
    for (NodeID v = 0; v < num_nodes; v++) {
      ...
      std::sort(adj.begin(), adj.end());
      int_adj_list.push_back(adj);
    }
}

void redu_vcc::addSolveCliques(std::vector<NodeID> &int_to_node_map, std::vector<std::vector<int>> &solve_cliques) {

  for (std::vector<int> solve_clique : solve_cliques) {
    std::vector<NodeID> clique;
    for (int n : solve_clique) {
      NodeID v = int_to_node_map[n];
      clique.push_back(v);
    }
...
void redu_vcc::solveReduced(graph_access &G, PartitionConfig &partition_config, timer &t) {

  if (remaining_nodes == 0) { return; }

  std::vector<int> node_to_int_map;
  node_to_int_map.resize(num_nodes);
  std::vector<NodeID> int_to_node_map;
  int_to_node_map.resize(num_nodes);
  std::vector<std::vector<int>> int_adj_list;
  unsigned long edges_count = 0;

  generateReducedAdjList(node_to_int_map, int_to_node_map, int_adj_list, edges_count);

  // buildKernel(G);

  cli *cli_instance;
  cli_instance = new cli(partition_config.seed, partition_config.mis);
  cli_instance->start_cli(int_adj_list, remaining_nodes, edges_count, t.elapsed(), partition_config.solver_time_limit);
  ...
  if (cli_instance->clique_cover.size() != 0){
      addSolveCliques(int_to_node_map, cli_instance->clique_cover);
