...
void Graph::read_graph(redu_vcc &reduVCC) {


	n = reduVCC.remaining_nodes;

	ui *from_map = new ui[reduVCC.num_nodes];
	to_map = new ui[n];

	ui v = 0;
	for (ui v_old = 0; v_old < reduVCC.num_nodes; v_old++) {
		if (!reduVCC.node_status[v_old]) continue;
		from_map[v_old] = v;
		to_map[v] = v_old;
		v++;

		for (ui u_old : reduVCC.adj_list[v_old]) {
			if (!reduVCC.node_status[u_old]) continue;
			m++;
		}
	}
	...
	ui i = 0;
	for(ui v = 0; v < reduVCC.num_nodes; v++) {
		if (!reduVCC.node_status[v]) continue;
		// if(degree[i] > 0) fread(buf, sizeof(int), degree[i], f);

		ui j = 0;
		for (ui u : reduVCC.adj_list[v]) {
			if (!reduVCC.node_status[u]) continue;
			ui new_u = from_map[u];

			edges[pstart[i] + j] = new_u;
			j++;
		}
...
ui Graph::degree_two_kernal_and_remove_max_degree_without_contraction() {
#ifndef NDEBUG
...
ui Graph::degree_two_kernal_dominate_lp_and_remove_max_degree_without_contraction() {
#ifndef NDEBUG
	ui *tmp_edges = new ui[m];
	for(ui i = 0;i < m;i ++) tmp_edges[i] = edges[i];
