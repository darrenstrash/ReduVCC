...
public:
	...
	ui degree_two_kernal_dominate_lp_and_remove_max_degree_without_contraction() ;
...
private:
	...
	int delete_vertex(ui v, char *is, int *degree, int *head, SigEdge *es, int *bin_head, int *bin_next, int *bin_pre, vector<ui> &degree_ones, vector<ui> &degree_twos) ;
};
