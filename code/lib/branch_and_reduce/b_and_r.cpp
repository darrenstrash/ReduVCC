...
branch_and_reduce::branch_and_reduce(graph_access &G, PartitionConfig &partition_config) {
  ...
  root_instance.reduVCC = redu_vcc(G, partition_config);
...
vertex_queue* branch_and_reduce::construct_queue(graph_access &G, instance &inst, std::vector<NodeID> &clique) {
  ...
  for (NodeID a : clique) new_queue->adjust_queue(inst.reduVCC, a);
  return new_queue;
...
void branch_and_reduce::bandr( graph_access &G, instance &curr_inst, unsigned int num_fold_cliques,
                               vertex_queue *queue,
                               PartitionConfig &partition_config, timer &t) {

  if (t.elapsed() > partition_config.solver_time_limit) return;

  redu_vcc &reduVCC = curr_inst.reduVCC;
  std::vector<reducer> &reducer_stack = curr_inst.reducer_stack;

  reducer R(G, partition_config.iso_limit);
  reduce(G, curr_inst, R, num_fold_cliques, queue);
  delete queue;
  ...
  if (prune(curr_inst, curr_cover_size)) {
    R.undoReductions(G, reduVCC); reducer_stack.pop_back();
    return;
  }
  ...
  NodeID next_node = nextNode(curr_inst);
  ...
  std::vector<std::vector<NodeID>> curr_cliques = enum_vertex(curr_inst, next_node);
  ...
  for (std::vector<NodeID> &clique : curr_cliques) {
    ...
    vertex_queue *new_queue = construct_queue(G, curr_inst, clique);
    ...
    // branch
    branch_count++;
    bandr(G, curr_inst, num_fold_cliques, new_queue, partition_config, t);
