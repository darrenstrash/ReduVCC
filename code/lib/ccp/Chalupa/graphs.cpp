...
void make_graph(std::vector<std::vector<int>> adj_list, unsigned int num_v, unsigned long num_e)
{
    ...
    for (unsigned int i = 0; i < adj_list.size(); i++){
        unsigned int edg_cnt = 0;
        for (unsigned int j = 0; j < adj_list[i].size(); j++){
            edg_cnt++;
        }
        G->V[i].edgecount = edg_cnt;
        G->V[i].sibl = (refer *) malloc (edg_cnt*sizeof(refer));
        for (unsigned int j = 0; j < edg_cnt; j++){
            G->V[i].sibl[j] = (unsigned int) adj_list[i][j];
        }
    }
