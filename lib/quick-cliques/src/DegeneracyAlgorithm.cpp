...
////    clock_t clockStart = clock();
    ...
    *pivotNonNeighbors = (int*)calloc(max(beginR-beginP,1), max(sizeof(int), q));
...
////        clock_t startClock = clock();
        ...
        *pNewBeginX = *pNewBeginP;
        ...
        while(j<orderingArray[orderNumber]->earlierDegree)
        {
            ...
            unsigned long int q = 1;
            neighborsInP[neighbor] = (int*)calloc(max(min(*pNewBeginR-*pNewBeginP,orderingArray[neighbor]->laterDegree),1), max(sizeof(int),q));
        ...
        while(j<*pNewBeginR)
        {
            ...
            neighborsInP[vertexInP]=(int*)calloc( max(min( *pNewBeginR-*pNewBeginP,
                                                 orderingArray[vertexInP]->laterDegree
                                               + orderingArray[vertexInP]->earlierDegree),1), max(sizeof(int),q));
...
long DegeneracyAlgorithm::listAllMaximalCliquesDegeneracy(vector<list<int>> const &adjList, int size)
{
    ...
    // |--X--|--P--|
    unsigned long int q = 1;
    int* vertexSets = (int*)calloc(max(size,1), max(sizeof(int),q));

    // vertex i is stored in vertexSets[vertexLookup[i]]
    int* vertexLookup = (int*)calloc(max(size,1), max(sizeof(int),q));

    int** neighborsInP = (int**)calloc(max(size,1), max(sizeof(int*),q));
    int* numNeighbors = (int*)calloc(max(size,1), max(sizeof(int),q));
...
////    clock_t timeDegeneracyOrder = clockEnd - clockStart;
    ...
    while(i<size)
    {
        vertexLookup[i] = i;
        vertexSets[i] = i;
        neighborsInP[i] = (int*)calloc(1, max(sizeof(int),q));
        numNeighbors[i] = 1;
        i++;
    }
