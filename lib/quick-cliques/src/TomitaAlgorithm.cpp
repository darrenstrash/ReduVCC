                                  ...
                                  int    numVertices )
{
    ...
    int* vertexSets = (int*)calloc(max(numVertices,1), max(sizeof(int),q));

    // vertex i is stored in vertexSets[vertexLookup[i]]
    int* vertexLookup = (int*)calloc(max(numVertices,1), max(sizeof(int),q));
                                     ...
                                     int beginX, int beginP, int beginR )
{
    ...
    *numNonNeighbors = 0;
    ...
    {
        unsigned long int q = 1;
        *pivotNonNeighbors = (int*)calloc(max(beginR-beginP-maxIntersectionSize,1), max(sizeof(int),q));
